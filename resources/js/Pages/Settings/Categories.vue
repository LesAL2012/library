...
function showFormEdit(id) {
    ...
    props.categories.data.map(element => {
        if (element.id == id) {
            form.id = id;
    ...
    document.querySelector('.form-category-' + id).classList.remove('hidden');
    document.querySelector('.table-category-' + id).classList.add('hidden');
...
function update(id) {
    ...
    /* router.post(route('categories.update', id), {
        _method: 'put',
        name: form.name,
    });
 */
...
function destroy(id) {
    ...
    ID - ${form.id}\n`)) {
        ...
        if (books_count == 0) {
            router.post(route('categories.destroy', form.id), {
                _method: 'delete',
            });
            reloadPage();

        } else {
            alert('This category has ' + books_count + ' books in the system, not NULL!\nThe number of books must be zero.');
        } btnShowAdd
    }
};
...
document.addEventListener('keydown', (event) => {
    if (event.code == 'Escape') {
        ...
        document.querySelectorAll('.form-category').forEach(element => {
            element.classList.add('hidden');
        });
        document.querySelectorAll('.table-category').forEach(element => {
            element.classList.remove('hidden');
        });
...
<template>
    ...
    <SettingsLayout>
        ...
        <h1 class="text-4xl py-2 text-center font-bold">Categories</h1>
        <p class="text-xs text-center text-red-700">Actions - Admin or Moderator</p>
        <Pagination class="w-full" :links="props.categories.links" />
            ...
            <tbody>
                    ...
                    <td>
                        ...
                        <form @submit.prevent="create()">
                            ...
                            <input id="id_name_create" :class="{ 'border-red-500': form.errors.name }" v-model="form.name"
                                placeholder="Add a new category name" type="text"
                ...
                <tr v-for="category in props.categories.data">
                    ...
                    <td class="w-96">
                        ...
                        <div :class="'table-category-' + category.id + ' table-category'">
                            {{ category.name }}
                        </div>

                        <form @submit.prevent="update(category.id)"
                            :class="'form-category-' + category.id + ' form-category hidden'">
                    ...
                    <td class="text-center w-48"
                        ...
                        <div class="action flex justify-around">
                            ...
                            <svg @click="showFormEdit(category.id)" xmlns="http://www.w3.org/2000/svg" fill="lightblue"
                            ...
                            <svg @click="destroy(category.id)" xmlns="http://www.w3.org/2000/svg" fill="lightcoral"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                class="w-8 h-8 inline rounded-md hover:bg-indigo-300 cursor-pointer">
                                <path stroke-linecap="round" stroke-linejoin="round"
